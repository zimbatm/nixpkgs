...
let
  ...
  systemdEntry = service: cfgFile: (
    mapAttrs' (
      name: cfg:
        (
          nameValuePair "beegfs-${service}-${name}" (
  ...
  systemdHelperd = mapAttrs' (
    name: cfg:
      (
        nameValuePair "beegfs-helperd-${name}" (
  ...
  config =
    mkIf config.services.beegfsEnable {
      ...
      environment.etc = mapAttrs' (
        name: cfg:
          (
            nameValuePair "beegfs/client-${name}.conf" (
              mkIf (cfg.client.enable)
                {
                  enable = true;
      ...
      fileSystems = mapAttrs' (
        name: cfg:
          (
            nameValuePair cfg.client.mountPoint (
              optionalAttrs cfg.client.mount (
                mkIf cfg.client.enable {
                  device = "beegfs_nodev";
                  fsType = "beegfs";
