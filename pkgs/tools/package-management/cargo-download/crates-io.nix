...
rec {
  ...
  crates.rand."0.6.1" = deps: { features ? (features_.rand."0.6.1" deps {}) }: buildRustCrate {
    ...
    ++ (
         if kernel == "cloudabi" then mapFeatures features (
           []
           ++ (if features.rand."0.6.1".cloudabi or false then [ (crates.cloudabi."${deps."rand"."0.6.1".cloudabi}" deps) ] else [])
         ) else []
       )
    ++ (
         if kernel == "fuchsia" then mapFeatures features (
           []
           ++ (if features.rand."0.6.1".fuchsia-zircon or false then [ (crates.fuchsia_zircon."${deps."rand"."0.6.1".fuchsia_zircon}" deps) ] else [])
         ) else []
       )
    ++ (
         if (kernel == "linux" || kernel == "darwin") then mapFeatures features (
           []
           ++ (if features.rand."0.6.1".libc or false then [ (crates.libc."${deps."rand"."0.6.1".libc}" deps) ] else [])
         ) else []
       )
    ++ (
         if kernel == "windows" then mapFeatures features (
           []
           ++ (if features.rand."0.6.1".winapi or false then [ (crates.winapi."${deps."rand"."0.6.1".winapi}" deps) ] else [])
         ) else []
       )
    ++ (
         if kernel == "wasm32-unknown-unknown" then mapFeatures features (
  ...
  crates.tar."0.4.13" = deps: { features ? (features_.tar."0.4.13" deps {}) }: buildRustCrate {
    ...
    ++ (
         if (kernel == "linux" || kernel == "darwin") then mapFeatures features (
           []
           ++ (if features.tar."0.4.13".xattr or false then [ (crates.xattr."${deps."tar"."0.4.13".xattr}" deps) ] else [])
         ) else []
       )
    ;
