...
lib.makeScope newScope (
  self: with self; {
    ...
    libxcb = callPackage (
      { stdenv, pkgconfig, fetchurl, libxslt, libpthreadstubs, libXau, xcbproto, libXdmcp, python }: stdenv.mkDerivation {
        name = "libxcb-1.13.1";
        builder = ./builder.sh;
        src = fetchurl {
    ...
    xcbproto = callPackage (
      { stdenv, pkgconfig, fetchurl, python }: stdenv.mkDerivation {
        name = "xcb-proto-1.13";
        builder = ./builder.sh;
        src = fetchurl {
