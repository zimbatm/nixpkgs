...
{ lib, external, pkgs }: variant: self: let
  ...
  dontConfigure = pkg: if pkg != null then pkg.override (
    args: {
  ...
  markBroken = pkg: if pkg != null then pkg.override (
    args: {
      melpaBuild = drv: args.melpaBuild (
        drv
        // {
    ...
    { archiveJson ? ./recipes-archive-melpa.json
      ...
      overrides = rec {
        shared = {
          ...
          cmake-mode = super.cmake-mode.overrideAttrs (
            attrs: {
          ...
          hindent = super.hindent.overrideAttrs (
            attrs: {
        ...
        unstable = shared
          // {
               ...
               irony = shared.irony.overrideAttrs (
                 old: {
                   meta = old.meta
                     // {
                          broken = true;
               ...
               vterm = let
                 ...
                 libvterm = pkgs.libvterm-neovim.overrideAttrs (
                   old: rec {
                 ...
                 args: {
                   melpaBuild = drv: args.melpaBuild (
                     ...
                     // {
                          prePatch =
                            let
                              w3m = "${lib.getBin external.w3m}/bin/w3m";
                            in
                              ''
