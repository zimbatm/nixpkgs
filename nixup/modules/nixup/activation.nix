    ...
    nixup.activationScripts = mkOption {
      ...
      apply = set:
        ''
          ...
          # FIX: XDG_RUNTIME has not been exported by systemd, yet.
          XDG_RUNTIME_DIR=/run/user/$(id -u $USER)

          # NixUP runtime directory.
          NIXUP_RUNTIME_DIR=$XDG_RUNTIME_DIR/nixup
          mkdir -m 0755 -p $NIXUP_RUNTIME_DIR

          # NixUP garbage collector roots directory.
          NIXUP_USER_GCROOTS_DIR=/nix/var/nix/gcroots/nixup/$USER
          ...
          # Allow to keep old profile for later use in, e.g., switch-systemd-services.pl
