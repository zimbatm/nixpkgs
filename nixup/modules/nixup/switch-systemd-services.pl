...
my $startListFile = "$ENV{'XDG_RUNTIME_DIR'}/nixup/systemd-start-list";
my $restartListFile = "$ENV{'XDG_RUNTIME_DIR'}/nixup/systemd-restart-list";
my $reloadListFile = "$ENV{'XDG_RUNTIME_DIR'}/nixup/systemd-reload-list";

my $action = shift @ARGV;

if (!defined $action || ($action ne "switch" && $action ne "login" && $action ne "test" && $action ne "dry-activate")) {
    print STDERR <<EOF;
Usage: $0 [switch|login|test]

switch:       make the configuration the login default and activate now
login:        make the configuration the login default
test:         activate the configuration, but don\'t make it the login default
...
system("@coreutils@/bin/sync") unless ($ENV{"NIXUP_NO_SYNC"} // "") eq "1";

exit 0 if $action eq "login";
...
sub getActiveUnits {
    ...
    my $lines = `LANG= systemctl --user list-units --full --no-legend`;
    my $res = {};
...
while (my ($unit, $state) = each %{$activePrev}) {
    ...
    my $prevUnitFile;
    my $newUnitFile;
    if (-e "/etc/systemd/user/$baseUnit") {
        $prevUnitFile = "/etc/systemd/user/$baseUnit";
        $newUnitFile = "/etc/systemd/user/$baseUnit";
    } else {
        $prevUnitFile = "$ENV{'XDG_RUNTIME_DIR'}/nixup/old-active-profile/systemd/$baseUnit";
...
syslog(LOG_NOTICE, "switching to nixup configuration $out");

if (scalar (keys %unitsToStop) > 0) {
    print STDERR "stopping the following units: ", join(", ", @unitsToStopFiltered), "\n"
        if scalar @unitsToStopFiltered;
    system("systemctl", "--user", "stop", "--", sort(keys %unitsToStop)); # FIXME: ignore errors?
}
...
if ($restartSystemd) {
    ...
    system("@systemd@/bin/systemctl", "--user", "daemon-reexec") == 0 or $res = 2;
}

# Forget about previously failed services.
system("@systemd@/bin/systemctl", "--user", "reset-failed");

# Make systemd reload its units.
system("@systemd@/bin/systemctl", "--user", "daemon-reload") == 0 or $res = 3;

# Reload units that need it. This includes remounting changed mount
# units.
if (scalar(keys %unitsToReload) > 0) {
    print STDERR "reloading the following units: ", join(", ", sort(keys %unitsToReload)), "\n";
    system("@systemd@/bin/systemctl", "--user", "reload", "--", sort(keys %unitsToReload)) == 0 or $res = 4;
...
if (scalar(keys %unitsToRestart) > 0) {
    ...
    system("@systemd@/bin/systemctl", "--user", "restart", "--", sort(keys %unitsToRestart)) == 0 or $res = 4;
...
print STDERR "starting the following units: ", join(", ", @unitsToStartFiltered), "\n"
    if scalar @unitsToStartFiltered;
system("@systemd@/bin/systemctl", "--user", "start", "--", sort(keys %unitsToStart)) == 0 or $res = 4;
...
while (my ($unit, $state) = each %{$activeNew}) {
    if ($state->{state} eq "failed") {
        ...
        my $lines = `@systemd@/bin/systemctl --user show '$unit'`;
        my $info = {};
...
if (scalar @failed > 0) {
    ...
    foreach my $unit (@failed) {
        print STDERR "\n";
        system("COLUMNS=1000 @systemd@/bin/systemctl --user status --no-pager '$unit' >&2");
    }
    $res = 4;
}
